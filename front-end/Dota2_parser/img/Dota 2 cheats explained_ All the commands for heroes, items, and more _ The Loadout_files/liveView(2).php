...
var SekindoClientDetections_URL = function (config) {
	...
	this.COOKIE_TIMEOUT = 25*24*60*60;
	...
	this._setObserverChecker = function()
	{
		try
		{
			...
			if (!this.isApp && !this.isAmpProject && (this.ci.browser == 'chrome' || this.ci.browser == 'edgeChromium')
	...
	this._observerChecker = function()
	{
		...
		if (ref.observerElement)
		{
			observerOptions = {
				root: null,
				rootMargin: '0px',
				threshold: [config.playerInViewPrc, 0.5]
			}
	...
	this.setInfo();
				...
				var urlDetObj = new SekindoClientDetections_URL({
					url: 'https://live.primis.tech/live/liveView.php?schain=1.0,1!network-n.com,pa_fec65292,1&cbuster=1630347587476&subId=[SUBID_ENCODED]&pubUrl=https%3A%2F%2Fwww.theloadout.com%2Fdota-2%2Fcheats&s=107160&cbuster=1630347602',
					origQString: 'schain=1.0,1!network-n.com,pa_fec65292,1&cbuster=1630347587476&subId=[SUBID_ENCODED]&pubUrl=https%3A%2F%2Fwww.theloadout.com%2Fdota-2%2Fcheats&s=107160',
					placementId: 107160,
					x: 600,
					y: 425,
					videoType: 'flow',
